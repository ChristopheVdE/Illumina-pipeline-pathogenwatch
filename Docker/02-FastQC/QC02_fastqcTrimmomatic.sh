...
for id in `cat /home/Pipeline/data/sampleList.txt`; do
     ...
     mkdir -p /home/Pipeline/data/${id}/03_QC-Trimmomatic_Paired/QC_FastQC
     #RUN FASTQC
     for i in $(ls /home/Pipeline/data/${id}/02_Trimmomatic | grep _P.fastq.gz); do
          echo -e "STARTING FastQC on paired reads of ${i} \n";
          fastqc --extract \
          -t ${threads} \
          -o /home/Pipeline/data/${id}/03_QC-Trimmomatic_Paired/QC_FastQC \
          /home/Pipeline/data/${id}/02_Trimmomatic/${i} \
          2>&1 | tee -a /home/Pipeline/data/${id}/03_QC-Trimmomatic_Paired/QC_FastQC/stdout_err.txt ;
          echo -e "\n ${i} FINISHED \n";
     done
done
#------------------------------------------------------------------------------------------------------------
